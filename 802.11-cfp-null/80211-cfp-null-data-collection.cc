...
main (int argc, char *argv[])
{
  ...
  std::vector<uint32_t> clientSizeVec {1, 5, 15, 25};
  std::vector<double> trafficRateVec {20, 100, 200};

  for(uint32_t run = 1; run <= Nruns; run++)
  {
    for(double percTraffic = 0.1; percTraffic <= 1; percTraffic += 0.4)
    {
      for(auto trafficRateIter = trafficRateVec.begin(); trafficRateIter != trafficRateVec.end(); trafficRateIter++) // metres
      {
        for(double errorRate = 0; errorRate <= 0.3; errorRate += 0.15) // microseconds
        {
          for(auto clientSizeIter = clientSizeVec.begin(); clientSizeIter != clientSizeVec.end(); clientSizeIter++) // microseconds
          {
            ...
            << ", Client size: " << *clientSizeIter << std::endl;
              ...
              Experiment experiment(run,percTraffic, *trafficRateIter, errorRate,
                *clientSizeIter, simTime, "80211_pcf_null_data_shorter.txt");
              experiment.Setup();
              experiment.Run();
          }
        }
